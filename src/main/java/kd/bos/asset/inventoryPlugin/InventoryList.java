...
//			DynamicObject usedeptObj = (DynamicObject) usedeptProp.getValue(billObj);
			DynamicObject assettypeObj = (DynamicObject)this.getModel().getValue("zsf_search_assettype"); 
			DynamicObject usedeptObj = (DynamicObject)this.getModel().getValue("zsf_search_usedept"); 
		...
		if (StringUtils.equals(source.getKey(), KEY_OK)) {
			...
			if (null != rows && rows.length > 0) {
				for (int i = 0; i < rows.length; i++) {
					...
					Long supplier = entity.getDynamicObject("zsf_supplier") != null ?entity.getDynamicObject("zsf_supplier").getLong("id") :0;
					Long usePerson = entity.getDynamicObject("zsf_headuseperson") != null ?entity.getDynamicObject("zsf_headuseperson").getLong("id") :0;
					//查询是否已存在未盘点的相同数据
