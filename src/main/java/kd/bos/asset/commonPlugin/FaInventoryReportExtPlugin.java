 ...
 * @Copyright:
 */
public class FaInventoryReportExtPlugin extends AbstractFormPlugin implements HyperLinkClickListener {
	...
	public void beforeDoOperation(BeforeDoOperationEventArgs args) {
		...
		if (StringUtils.equals("audit", formOperate.getOperateKey())) {
			// TODO 在此添加业务逻辑
			Object ob = formOperate.getView().getModel();
			...
			if (scheme == null) {
				getView().showTipNotification(
						ResManager.loadKDString("未选择盘点方案", "FaInventoryReportPlugin_2", "fi-fa-formplugin", new Object[0]));
				return;
			}
...
//		vs.addField("zsf_audit", new Object[0]);
		int allTotal = 0;
		int allHasInvent = 0;
		...
		for (int i = 0; i < length; i++) {
			...
			vs.set("operationcolumn",
					ResManager.loadKDString("盘点记录", "FaInventoryReportPlugin_3", "fi-fa-formplugin", new Object[0]), i);
//			vs.set("zsf_audit",
//					ResManager.loadKDString("审核通过", "FaInventoryReportPlugin_3", "fi-fa-formplugin", new Object[0]), i);
		...
		if (("hide".equals(getPageCache().get("showLength"))) && (5 < length)) {
			...
			for (int j = 0; j < 5; j++) {
				...
				vs2.set("operationcolumn",
						ResManager.loadKDString("盘点记录", "FaInventoryReportPlugin_3", "fi-fa-formplugin", new Object[0]),
						j);
			}
			vs = vs2;
		}
	...
	private boolean hasInventoryScheme() {
		...
		if ((orgIds.isEmpty()) && (!permOrgRs.hasAllOrgPerm())) {
			getView().showTipNotification(ResManager.loadKDString("未找到有权限的核算组织，无法找到可访问的盘点方案",
					"FaInventoryReportPlugin_4", "fi-fa-formplugin", new Object[0]));
			return false;
		}
		return true;
	}
	...
	public SchemeEntry getTaskInventoryState(String taskID) {
		...
		String sql = "select FID,FENTRYID,FTASKRULE,FNAME,FK_ZSF_INVENTORYSTATE  from t_fa_inventschemeentry "
				+ " where FENTRYID =(select fentryid from t_fa_invent_taskrule where FDETAILID =?) ";
